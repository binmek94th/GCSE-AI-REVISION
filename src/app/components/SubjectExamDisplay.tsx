...
export default function SubjectExamDisplay({ initialPacks }: { initialPacks: ExamItem[] }) {
    ...
    // Data arrives pre-fetched from the server component — no client fetch, no loading/error state needed
    const levelData: ExamItem[] = useMemo(
        () => initialPacks.filter(e => e.level === selectedLevel),
        [initialPacks, selectedLevel]
    );
